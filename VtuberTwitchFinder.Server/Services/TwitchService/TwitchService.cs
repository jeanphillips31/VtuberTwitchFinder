...
public class TwitchService : ITwitchService
{
    ...
    public async Task<Result<DTStreamerEmotes>> GetChannelEmotes(int broadcasterId)
    {
        try
        {
            ...
            if (messageResponse.StatusCode != HttpStatusCode.OK) return Result.Fail("Failed to get emotes from Twitch API");
            ...
            //Add Follower emotes
            emotes.FollowerEmotes = responseContent.Data.Where(emote => emote.emote_type == "follower").Select(e => new DTEmote
            {
                Id = e.id,
                Name = e.name,
                Url = e.images.url_4x
            ...
            emotes.SubscriptionEmotes.Tier1Emotes = responseContent.Data.Where(emote => emote.emote_type == "subscriptions" & emote.tier == "1000")
                .Select(
                    e =>
                        new DTEmote
                        {
            ...
            emotes.SubscriptionEmotes.Tier2Emotes = responseContent.Data.Where(emote => emote.emote_type == "subscriptions" & emote.tier == "2000")
                .Select(
                    e =>
                        new DTEmote
                        {
            ...
            emotes.SubscriptionEmotes.Tier3Emotes = responseContent.Data.Where(emote => emote.emote_type == "subscriptions" & emote.tier == "3000")
                .Select(
                    e =>
                        new DTEmote
                        {
    ...
    public async Task<Result<IEnumerable<DTTwitchClip>>> GetChannelClips(int broadcasterId)
    {
        ...
        string startDate = ConvertDateTimeToRFC3339Format(DateTime.UtcNow.AddDays(-30));
        string endDate = ConvertDateTimeToRFC3339Format(DateTime.UtcNow);
        //Generate request and send it
        var message = new HttpRequestMessage(HttpMethod.Get,
        ...
        //Deserialize data
        var responseContent = await messageResponse.Content.ReadFromJsonAsync<DTTwitchResponse<DTTwitchClipResponse[]>>();
        if (responseContent == null) return Result.Fail("Failed to deserialize TwitchResponse");
